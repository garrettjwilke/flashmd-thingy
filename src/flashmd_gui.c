...
static const uint32_t size_values[] = {0, 512, 1024, 2048, 4096, 8192};
...
static void *worker_thread_func(void *arg) {
    ...
    switch (state.current_operation) {
        ...
        case OP_ERASE:
            result = flashmd_erase(state.op_size_kb, &config);
            break;
 ...
 */
static bool draw_button(Rectangle bounds, const char *text) {
    bool hover = CheckCollisionPointRec(GetMousePosition(), bounds);
    bool pressed = hover && IsMouseButtonPressed(MOUSE_LEFT_BUTTON);

    Color bg = hover ? current_theme->button_hover : current_theme->button_bg;
    ...
    draw_text(text, text_x, text_y, FONT_SIZE, current_theme->button_text);
 ...
 */
static bool draw_checkbox(Rectangle bounds, const char *label, bool *checked) {
    bool hover = CheckCollisionPointRec(GetMousePosition(), bounds);
    ...
    /* Draw box */
    DrawRectangleRounded(bounds, 0.2f, 4, current_theme->input_bg);
    DrawRectangleRoundedLinesEx(bounds, 0.2f, 4, 1, current_theme->panel_border);
    ...
    if (*checked) {
        DrawRectangleRounded(
            ...
            0.2f, 4, current_theme->accent);
    }

    /* Draw label */
    draw_text(label, (int)(bounds.x + bounds.width + 8), (int)(bounds.y + 2),
              FONT_SIZE, current_theme->text_secondary);
...
static bool draw_dropdown(Rectangle bounds, const char **options, int option_count,
    ...
    if (*active) {
        ...
        for (int i = 0; i < option_count; i++) {
            Rectangle item = {bounds.x, list_y + i * 28, bounds.width, 26};
            bool item_hover = CheckCollisionPointRec(GetMousePosition(), item);

            Color item_bg = item_hover ? current_theme->button_hover : current_theme->panel;
            DrawRectangleRec(item, item_bg);

            draw_text(options[i], (int)item.x + 8, (int)(item.y + 5),
                      FONT_SIZE, current_theme->text_primary);
        ...
        DrawRectangleLinesEx(
            (Rectangle){bounds.x, list_y, bounds.width, (float)(option_count * 28)},
...
int main(void) {
    ...
    const char *size_opts[] = {"Auto (4MB)", "512 KB", "1 MB", "2 MB", "4 MB", "8 MB"};
    int size_opt_count = 6;
    ...
    while (!WindowShouldClose()) {
        ...
        if (draw_button((Rectangle){(float)(width - 60), (float)y, 70, 30}, theme_label)) {
        ...
        if (draw_button((Rectangle){(float)(width - 195), (float)(y + 12), 90, 32}, "Connect")) {
            if (!running) {
                console_add_line("");
                start_operation(OP_CONNECT);
            }
        }
        if (draw_button((Rectangle){(float)(width - 95), (float)(y + 12), 85, 32}, "Check ID")) {
            if (!running) {
                console_add_line("");
        ...
        draw_input_field((Rectangle){(float)(margin + 60), (float)(y + 2), (float)(width - 180), 32},
                         state.rom_filepath, "(no file selected)");
        if (draw_button((Rectangle){(float)(width - 100), (float)(y + 2), 90, 32}, "Browse")) {
            if (!running) open_rom_file_dialog(0);
        }
        y += row_height + 2;

        /* Size row */
        int size_row_y = y;
        draw_text("Size:", margin + 14, y + 8, FONT_SIZE, current_theme->text_primary);
        /* Checkbox */
        draw_checkbox((Rectangle){(float)(margin + 220), (float)(y + 7), 22, 22}, "No trim", &state.no_trim);
        y += row_height + 2;

        /* ROM action buttons row */
        if (draw_button((Rectangle){(float)(margin + 14), (float)y, 100, 34}, "Read ROM")) {
            if (!running) {
        ...
        if (draw_button((Rectangle){(float)(margin + 122), (float)y, 100, 34}, "Write ROM")) {
            if (!running && state.rom_filepath[0]) {
                console_add_line("");
                start_operation(OP_WRITE_ROM);
            } else if (!state.rom_filepath[0]) {
                console_add_line("Please select a ROM file first");
            }
        }
        if (draw_button((Rectangle){(float)(margin + 230), (float)y, 85, 34}, "Erase")) {
            if (!running) {
                console_add_line("");
        ...
        if (draw_button((Rectangle){(float)(margin + 14), (float)y, 105, 34}, "Read SRAM")) {
            if (!running) {
        ...
        if (draw_button((Rectangle){(float)(margin + 127), (float)y, 105, 34}, "Write SRAM")) {
        ...
        /* Bottom buttons */
        if (draw_button((Rectangle){(float)margin, (float)y, 80, 32}, "Clear")) {
            console_clear();
        }

        draw_checkbox((Rectangle){(float)(width - 90), (float)(y + 6), 22, 22}, "Verbose", &state.verbose_mode);
        ...
        draw_dropdown((Rectangle){(float)(margin + 60), (float)(size_row_y + 4), 145, 30},
                      size_opts, size_opt_count, &state.rom_size_index, &state.size_dropdown_active);
